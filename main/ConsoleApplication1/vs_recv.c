...
int rudp_receiver(rudp_socket_t rsocket, struct sockaddr_in *remote, char *buf, int len) {
  ...
  strncpy_s((char *)rx->name, sizeof(char), (const char *)vs->vs_info.vs_filename, namelen);
  ...
  if ((rx->fd = _creat(rx->name, 0644)) < 0) {
  ...
  if (rx->fileopen) {
    if ((_write(rx->fd, vs->vs_info.vs_filename, len)) < 0) {
